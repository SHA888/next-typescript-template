...
export class JwtAuthGuard extends AuthGuard("jwt") {
  ...
  canActivate(
    ...
    if (this.isTestEnvironment) {
      ...
      if (authHeader && authHeader.startsWith("Bearer ")) {
        ...
        this.logger.log(
          `Test token: ${token ? "***" + token.slice(-8) : "NO TOKEN"}`,
        );
      ...
      this.logger.warn(
        "No auth token in test environment - request will be unauthenticated",
      );
      return true;
    }
  ...
  handleRequest(err: any, user: any, info: any, context: ExecutionContext) {
    ...
    this.logger.log(
      `User received: ${user ? JSON.stringify(user, null, 2) : "None"}`,
    );
    ...
    if (this.isTestEnvironment) {
      this.logger.warn(
        "Running in TEST environment - applying test-specific user handling",
      );

      // If we have a user object, return it even if there's an error
      if (user) {
        this.logger.warn(
          "Test environment: Returning user despite potential errors",
        );
        return user;
      }
